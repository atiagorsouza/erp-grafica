import { deletePrinter, savePrinter } from "@/lib/print-engine";
...
export async function POST(req: Request) {
  ...
  try {
    if (op === "create") return jsonResult(await savePrinter(data));
    if (op === "update") {
      if (!Number.isFinite(id)) return Response.json({ error: "id obrigatório" }, { status: 400 });
      return jsonResult(await savePrinter(data, id));
    }
    if (op === "delete") {
      if (!Number.isFinite(id)) return Response.json({ error: "id obrigatório" }, { status: 400 });
      return jsonResult(await deletePrinter(id));
    }
