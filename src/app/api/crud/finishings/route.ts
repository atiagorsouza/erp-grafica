import { archiveFinishing, saveFinishing } from "@/lib/services-engine";
...
export async function POST(req: Request) {
  ...
  try {
    if (op === "create") return jsonResult(await saveFinishing(data));
    if (op === "update") {
      if (!Number.isFinite(id)) return Response.json({ error: "id obrigatório" }, { status: 400 });
      return jsonResult(await saveFinishing(data, id));
    }
    if (op === "delete" || op === "archive") {
      if (!Number.isFinite(id)) return Response.json({ error: "id obrigatório" }, { status: 400 });
      return jsonResult(await archiveFinishing(id, String(data.reason || "Arquivado pelo usuário")));
    }
    return Response.json({ error: "op inválido" }, { status: 400 });
  } catch (e) {
    console.error("[finishings]", e);
