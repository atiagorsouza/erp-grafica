import { archivePricingTable, createPricingTable, updatePricingTable } from "@/lib/pricing-tables";
...
export async function POST(req: Request) {
  ...
  try {
    if (op === "create") return jsonResult(await createPricingTable(data));
    if (op === "update") {
      if (!Number.isFinite(id)) return Response.json({ error: "id obrigatório" }, { status: 400 });
      return jsonResult(await updatePricingTable(id, data));
    }
    if (op === "delete" || op === "archive") {
      if (!Number.isFinite(id)) return Response.json({ error: "id obrigatório" }, { status: 400 });
      return jsonResult(await archivePricingTable(id));
    }
    return Response.json({ error: "op inválido" }, { status: 400 });
  } catch (e) {
    console.error("[pricing-tables]", e);
