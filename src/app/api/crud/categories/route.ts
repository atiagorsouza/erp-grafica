import { deletePrinterCategory, savePrinterCategory } from "@/lib/print-engine";
...
export async function POST(req: Request) {
  ...
  try {
    if (op === "create") return jsonResult(await savePrinterCategory(data));
    if (op === "update") {
      if (!Number.isFinite(id)) return Response.json({ error: "id obrigatório" }, { status: 400 });
      return jsonResult(await savePrinterCategory(data, id));
    }
    if (op === "delete") {
      if (!Number.isFinite(id)) return Response.json({ error: "id obrigatório" }, { status: 400 });
      return jsonResult(await deletePrinterCategory(id));
    }
    return Response.json({ error: "op inválido" }, { status: 400 });
  } catch (e) {
    console.error("[printer-categories]", e);
