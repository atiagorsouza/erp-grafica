...
export async function reorderKanban(column: KanbanColumn, ids: number[]) {
  ...
  const rows = await db.select({ id: kanbanCards.id }).from(kanbanCards).where(inArray(kanbanCards.id, ids));
